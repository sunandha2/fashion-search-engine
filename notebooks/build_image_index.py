...
# Focus on Apparel + Footwear for image search
# These have the most visually distinctive images
...
def search_by_image(image_path, top_k=5):
    ...
    # Skip the first result (it's the query image itself)
    results = df_visual.iloc[valid_indices].iloc[indices[0][1:]].copy()
...
def search_by_text_clip(text_query, top_k=5):
    ...
    results = df_visual.iloc[valid_indices].iloc[indices[0]].copy()
    results['similarity_score'] = scores[0]
    return results[['productDisplayName', 'articleType',
                     'baseColour', 'gender', 'similarity_score']]

# Test CLIP text search on image index
...
# Test image → image search with first product
print("\nTesting image → image search:")
test_image = df_visual.iloc[0]['image_path']
test_product = df_visual.iloc[0]['productDisplayName']
...
# Save valid product metadata aligned with image index
...
print(f"\nTotal images in index: {image_index.ntotal}")
